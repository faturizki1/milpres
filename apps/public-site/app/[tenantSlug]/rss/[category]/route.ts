...
export async function GET(request: NextRequest, { params }: { params: Promise<{ tenantSlug: string; category: string }> }) {
  const { tenantSlug, category } = await params
  const contents = await fetchTenantContents(tenantSlug)
  const hostname = process.env.PUBLIC_SITE_HOST || 'https://example.com'
  const filtered = contents.filter((item) => item.category?.slug === category || item.category?.name.toLowerCase() === category.toLowerCase())

  const rss = `<?xml version="1.0" encoding="UTF-8" ?>
<rss version="2.0">
  <channel>
    <title>RSS ${tenantSlug} - ${category}</title>
    <link>${hostname}/${tenantSlug}/rss/${category}</link>
    <description>Feed kategori ${category}</description>
